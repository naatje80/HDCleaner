...
elif [[ -n ${FORM_CE_SUPPORTED} ]]
then
	if [[ `nvme id-ctrl ${1} -H|grep -E 'Format Applies to Single Namespace(s)|Crypto Erase Applies to Single Namespace(s)'|grep -c -v '0x1' == '2' ]]
	then
		ERASE_COMMAND="nvme format ${1} -s 2 -n 1"
	fi
else
	ERASE_COMMAND="bkjdiscard --secure ${1}"
if
